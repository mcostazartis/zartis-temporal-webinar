...
// returns error activity.ErrResultPending, the temporal client recognize this error, and won't mark this activity
...
func WaitForDecisionActivity(ctx context.Context, expenseID string) (string, error) {
	...
	if err != nil {
		logger.Info("waitForDecisionActivity failed to register callback.", err)
		return "", err
	}
	body, err := ioutil.ReadAll(resp.Body)
	resp.Body.Close()
